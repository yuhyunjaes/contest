...
SELECT i.id, i.category, i.img, i.name, i.descript, i.price, i.post, i.boon, DATE_FORMAT(i.date, '%Y-%m-%d') AS date, c.hap FROM item i LEFT JOIN discount c ON i.id = c.id AND i.category = c.category WHERE i.category = '".$whcate."'