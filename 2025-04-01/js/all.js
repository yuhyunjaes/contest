...
window.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.item_price').forEach(item=>{
        let total_price_hap = 0;
        total_price_hap = Number(item.innerText);
        item.textContent = total_price_hap.toLocaleString()+'원';
    })
    document.querySelectorAll('.itpost').forEach(item=>{
        let total_item_post = 0;
        total_item_post = Number(item.innerText);
        item.textContent = total_item_post.toLocaleString();
    })
    document.querySelectorAll('.del').forEach(item=>{
        let total_item_post = 0;
        total_item_post = Number(item.innerText);
        item.textContent = total_item_post.toLocaleString()+'원';
    })
